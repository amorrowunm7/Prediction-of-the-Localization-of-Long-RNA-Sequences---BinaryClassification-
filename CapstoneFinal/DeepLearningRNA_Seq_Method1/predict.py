...
def create_cnn_rna_seq(input_length, rnn_hidden_dim=RNN_HIDDEN_DIM, output_dim=OUTPUT_DIM, input_dim=INPUT_DIM,
                       dropout=DROPOUT_RATIO):
    model = Sequential()
    # model.add(Embedding(input_dim = INPUT_DIM, output_dim = output_dim, input_length = input_length, name='embedding_layer'))
    # output_dim
...
if __name__ == '__main__':
    ...
    print(X_test.shape)  # -> (2133, 56)
    print(y_test.shape)  # -> (2133, 2)

    model = create_conv_rna_seq(len(X_train[0]))
